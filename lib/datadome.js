...
const getNewToken = page => new Promise((resolve, reject) => {
  let timeoutId = -1

  page.on('response', async response => {
    if (response.url().startsWith('https://geo.captcha-delivery.com/captcha/check?cid=')) {
  ...
  solveGeetestCaptcha(page, frame)
    .then(() => {
      // In case response is not resolved
