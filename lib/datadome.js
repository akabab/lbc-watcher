...
const moveSlider = async (page, frame) => {
  ...
  if (G_MAKE_IT_FAIL) { xPosition += 20 }
...
const solve = async (page, frame) => {
  ...
  switch (state) {
    ...
    case 'geetest_radar_click_ready': {
      ...
      const state = await getState(frame)
      if (process.env.DEBUG) { console.log('AFTER REFRESH STATE', { state }) }
...
const solveGeetestCaptcha = async (page, frame, afterNthTry = 1) => {
  ...
  while (tries++ < maxTries) {
    if (tries >= afterNthTry) {
      G_MAKE_IT_FAIL = false
    }

    try {
      if (await solve(page, frame)) {
        return tries
      }
    } catch (error) {
