...
const cookiesHandler = async watcher => {
  ...
  await page.click('button#didomi-notice-disagree-button', { delay: getRandomInt(100, 300) })
...
const datadomeHandler = async watcher => {
  if (G_DD_LOCK) { 
    console.log(`[${formatWatcherIdentifier(watcher)}] Datadome: token already beeing retrieved, Pause watcher..`)
    watcher._PAUSED_FOR_DATADOME_TOKEN = true
    return
  }
  ...
  console.log(`[${formatWatcherIdentifier(watcher)}] Datadome solving Geetest, retrieving new cookie token...`)
  const cookie = await Datadome.getNewToken(newPage)

  // Add the new freshly retrieved token to the actual watcher page
  await page.evaluate(cookie => { document.cookie = cookie }, cookie)

  newPage.close()
  
  console.log(`[${formatWatcherIdentifier(watcher)}] Datadome solving Geetest succeed, new token ${cookie} added!`)

  // restart all paused watchers + the actual one
  watcherHandler(watcher)
  
  const watchers = Db.getAllWatchers()
  watchers.filter(w => w._PAUSED_FOR_DATADOME_TOKEN === true).map(w => {
    w._PAUSED_FOR_DATADOME_TOKEN = false
    watcherHandler(w)
  })
...
const watcherHandler = async watcher => {
  ...
  if (!watcher.active || watcher._SHOULD_BE_DELETED) {
    console.error(`[${formatWatcherIdentifier(watcher)}] !watcher.active || watcher._SHOULD_BE_DELETED, aborting watcher...`)
    return
  }

  if (watcher._page.isClosed() && watcher.active) {
    console.error(`[${formatWatcherIdentifier(watcher)}] Page may have been closed manually or crashed, restarting watcher...`)
  ...
  try {
    ...
    if (watcher._retries > Env.WATCHER_MAX_RETRIES_BEFORE_ABORT) {
      console.error(`[${formatWatcherIdentifier(watcher)}] Too much retries, aborting watcher...`)
